 ...
 const updateCardStatus = (selected: number) => {
  ...
  const displayBoard = () => {
    return board.map((card: Card, index: number) =>
      <FlipCard index={index} value={card.value} isFlipped={card.isFlipped} isLocked={card.isLocked} isDisabled={disable} handleSelect={handleSelect} /> 
    )
  }
